...
- The Update Center is another child mode of the menu binary. Its bounded newline-delimited IPC can request acknowledged bridge suspension, restart, or final shutdown; checking and downloading do not pause the runtime. The parent restarts a previously running bridge if the child exits while a firmware session owns the suspension.
