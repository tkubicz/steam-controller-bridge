...
- `sc-lizard-lab` is a developer-only measurement CLI. macOS capture keeps lizard mode enabled and merges controller HID plus a passive HID event tap; portable analysis, comparison, and dump replay consume both its additive v1 events and older raw recordings. Desktop replay is pointer-motion-only.
- `sc-visualizer` owns the optional `eframe` GUI. A dedicated HID polling thread feeds a 64-report `InputMailbox`; the UI drains it into decoder, mapper, background recording, and mock-output diagnostics without introducing GUI dependencies into any library or CLI. While there is room it preserves every report. Only under pressure does it remove redundant analog samples inside a run with the same 32-bit button mask; if no safe sample exists, overflow keeps the newest report as a recovery baseline so the display cannot stick on a stale press. Lifecycle transitions stay ordered, repeated identical backend errors are coalesced, and every report loss is counted. `--demo-state neutral|digital|analog|disconnected` drives the same mapping and rendering path from explicit decoded fixtures for deterministic visual checks; it does not claim fabricated raw wire bytes.
