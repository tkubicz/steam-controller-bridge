...
- `release-updater` is an internal security and state-machine boundary for exact-byte Ed25519 manifest verification, semantic compatibility and rollback checks, atomic caching, bounded HTTPS downloads, application staging, XIAO/UF2 identification, flashing, and post-flash revision verification. It exposes no supported external API.
...
- About, Changelog, and Updates share one child mode of the menu binary. The menu routes About and Check for Updates to the requested tab in the existing window and pushes firmware-revision changes into its hero. Its bounded newline-delimited IPC can also request acknowledged bridge suspension, restart, or final shutdown; checking and downloading do not pause the runtime. The parent restarts a previously running bridge if the child exits while a firmware session owns the suspension.
