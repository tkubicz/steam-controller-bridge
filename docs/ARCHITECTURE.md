...
slot producing complete state reports. The integrated runtime then runs that
session behind a bounded standard-library channel with latest-state semantics
for reports while preserving lifecycle events. A lost slot returns to discovery
instead of trusting a stale numeric index.
