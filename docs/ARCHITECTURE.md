...
- `sc-visualizer` owns the optional `eframe` GUI. A dedicated HID polling thread feeds a bounded 64-slot `InputMailbox`; the UI drains it into decoder, mapper, recording, and mock-output diagnostics without introducing GUI dependencies into any library or CLI. The mailbox mirrors `bridge-runtime`'s `TransitionReportMailbox`: under pressure it coalesces only redundant analog reports inside a run with the same 32-bit button mask, never discards a lifecycle event, and on overflow keeps the newest report as a recovery baseline, so the display cannot stick on a stale press. It counts what it coalesced and dropped rather than losing reports silently. `--demo-state neutral|digital|analog|disconnected` drives the same decode path with a fixed state for deterministic visual checks.
