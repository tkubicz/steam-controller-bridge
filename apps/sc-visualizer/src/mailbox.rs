...
impl InputMailbox {
    pub(crate) fn publish(&self, event: InputEvent) {
        ...
        let replaceable = state.run.can_replace_latest(mask)
            && matches!(state.events.back(), Some(InputEvent::Report(_)));
        if replaceable {
    ...
    /// Drains everything queued. The newest report is always in the result.
    pub(crate) fn take_all(&self) -> (Vec<InputEvent>, bool) {
        let mut state = self
            .state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        state.run.reset();
        (
            std::mem::take(&mut state.events).into(),
...
mod tests {
    ...
    fn under_capacity_every_report_is_retained() {
        ...
        // Distinct masks are never redundant, so nothing may be coalesced.
        for buttons in 0..u32::try_from(CAPACITY).unwrap() {
            mailbox.publish(InputEvent::Report(report(buttons)));
        }
    ...
    fn mailbox_coalesces_analog_runs_but_preserves_button_edges() {
        let mailbox = InputMailbox::default();
        // A run of three identical masks: the third replaces the second, so the
        // first and newest samples both survive.
        for _ in 0..3 {
        ...
        assert_eq!(coalesced, 1, "one redundant analog sample");
        ...
        let masks: Vec<Option<u32>> = events
            ...
            .collect();
        assert_eq!(masks, vec![Some(0b1), Some(0b1), Some(0b11)]);
    }
    ...
    fn draining_always_exposes_the_newest_report() {
        ...
        let (events, _) = mailbox.take_all();
        match events.last() {
            Some(InputEvent::Report(report)) => assert_eq!(button_mask(report), Some(0b1000)),
